...
| `anyOf`                                                                                                             | this takes an array of inputs and matches any of them, and you can use `anyOf.group()` to capture as an anonymous group.                                            |
...
| `or`                                          | this provides an alternative to the current input, and you can use `or.group()` to capture both inputs as an anonymous group.                                                                                                                                                                            |
...
| `group`                                       | this capture the entire input so far as an anonymous group.                                                                                                                                                                                                                                              |
| `at`                                          | this allows you to match beginning/ends of lines with `at.lineStart()` and `at.lineEnd()`.                                                                                                                                                                                                               |

::alert
By default, creation input helpers such as `anyOf`, `maybe`, `oneOrMore`, and chaining input helpers such as `or`, `times(.between/atLeast/any)`, `optionally` wraps input(s) in non-capturing group with `(?:` and `)`. To capure as an anonymous group, use `anyOf.group`, `or.group` or chaining `.group` after any `Input` type.
::
